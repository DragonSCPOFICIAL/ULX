...
# 4. Verificar dependências de compilação (GCC) e instalar se necessário
echo "[4/4] Verificando dependências de compilação (GCC) e instalando se necessário..."
if ! command -v gcc &> /dev/null; then
    ...
    if command -v pacman &> /dev/null; then
        ...
        echo "Aviso: Gerenciador de pacotes não reconhecido. Por favor, instale o GCC manualmente."
    fi
fi
